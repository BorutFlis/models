...
complete_results_df = pd.concat(gather_results, ignore_index=True).rename(columns={"Sensitivity (Recall)": "Sensitivity"})
# aggregate the metrics by folds
grouped_df = (
    complete_results_df.groupby(["target", "model", "imputer"])[["AUC", "Accuracy", "Sensitivity", "Specificity", 'Precision', 'F1 Score']].mean().reset_index()
)

# if you want to compare different targets
best_models_df = grouped_df.groupby(["target"]).apply(lambda x: x.loc[x["AUC"].idxmax()])

model_comparison = grouped_df.groupby("model").apply(lambda x: x.loc[x["AUC"].idxmax()])
model_comparison.round(3).loc[:, ["Accuracy", "AUC", "Sensitivity", "Specificity", 'Precision', 'F1 Score']]