...
full_df = pd.concat(gather_all_dfs)
mortality_data = full_df.loc[full_df.loc[:, full_df.columns.str.startswith("summary_Blo")].count(axis=1).ge(12)]
...
pred_by_quantile = pd.qcut(y_pred, q=5, labels=False)
for q_i in sorted(pred_by_quantile.unique()):
    quantile_i_filter = pred_by_quantile.eq(q_i)
    surv_i = rsf.predict_survival_function(X.loc[quantile_i_filter], return_array=True).mean(axis=0)

    plt.step(rsf.unique_times_[rsf.unique_times_ < 2500], surv_i[rsf.unique_times_ < 2500], where="post", label=f"Q{q_i + 1}")
