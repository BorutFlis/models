...
from disease_progression.data_loader.source import PatientLevelDataSource

DATA_DIR = "../data"

df = load_data(os.path.join(DATA_DIR, "raw",  "patient_level_dp.csv"))
df = df.iloc[:15000]
data_source = PatientLevelDataSource(df)
    ...
    for model_name in classifiers.keys():
        ...
        for imputer_name, imputer in imputers.items():
            ...
            for i, (train_index, test_index) in enumerate(cv_split(X, y)):
                ...
                inner_data_source = PatientLevelDataSource(df.iloc[train_index])
