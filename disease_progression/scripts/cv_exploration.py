...
mortality_data = pd.read_csv(os.path.join(DATA_DIR, "raw", "mortality_individual.csv"), index_col=0)
...
X = pd.DataFrame(median_imputer.fit_transform(X))
...
for i, (train_index, test_index) in enumerate(loo.split(X)):
    ...
    risk_score = rsf.predict(X_test)[0]
...
print(f"Leave-One-Out C-index: {cindex:.3f}")
